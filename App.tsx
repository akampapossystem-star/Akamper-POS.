...
import { AppView, SystemConfig, UserRole, Order, Product, StaffMember, Customer, RegisterState, Table, Expense, StockMovementLog, Requisition, BusinessPage, ChatMessage, StoreItem, RolePermissions, PermissionAction, ReturnRecord, OrderItem, AttendanceRecord, SpiritBottle, SectionAllocation, SystemAuditLog } from './types';
...
const App: React.FC = () => {
  ...
  const [auditLogs, setAuditLogs] = useIsolatedState<SystemAuditLog[]>(tId, 'auditLogs', []);
  ...
  // --- LOGGING HELPER ---
  const addAuditEntry = useCallback((action: SystemAuditLog['action'], details: string, metadata?: any) => {
  ...
  // --- CENTRAL INVENTORY BROKER ---
  const performCentralInventoryDeduction = useCallback((order: Order) => {
    console.log("[Inventory Broker] Triggering deductions for Order:", order.id);
    
    let currentStoreItems = [...storeItems];
    let newLogs: StockMovementLog[] = [];

    order.items.forEach(orderItem => {
        const product = products.find(p => p.id === orderItem.product.id);
        if (product?.composition) {
            product.composition.forEach(comp => {
                const storeItemIdx = currentStoreItems.findIndex(si => si.id === comp.itemId);
                if (storeItemIdx >= 0) {
                    const storeItem = currentStoreItems[storeItemIdx];
                    const qtyToDeduct = comp.quantity * orderItem.quantity;
                    const prevStock = storeItem.stock;
                    const newStock = prevStock - qtyToDeduct;

                    currentStoreItems[storeItemIdx] = { 
                        ...storeItem, 
                        stock: newStock, 
                    ...
                    newLogs.push({
                        id: `LOG-RECIPE-${Date.now()}-${storeItem.id}`,
                        itemId: storeItem.id,
                        ...
                        newStock: newStock,
                        reason: `Sale: ${product.name} (Recipe Fulfillment)`,
                        performedBy: 'System Auto-Deduct',
                        timestamp: new Date(),
                        destination: product.category.includes('Bakery') ? 'Bakery' : 'Kitchen'
                    });
                }
            });
        }
    ...
    if (newLogs.length > 0) {
        ...
        notify(`Bulk Stock updated for ${newLogs.length} ingredients`, 'success');
    }
  }, [storeItems, products, setStoreItems, setStockLogs]);
  ...
  const handleUpdatePermissions = (role: UserRole, type: 'view' | 'action', key: string, value: boolean) => {
    ...
    addAuditEntry('PERMISSION_CHANGE', `Updated permissions for ${role}`);
  ...
  const handlePlaceOrder = (order: Order) => {
      ...
      if (order.status === 'paid') {
          performCentralInventoryDeduction(order);
      }
  };
  ...
  const handleUpdateOrder = (updatedOrder: Order) => {
      ...
      if (updatedOrder.status === 'paid' && prevOrder?.status !== 'paid') {
          performCentralInventoryDeduction(updatedOrder);
      }
  };
  ...
  const handleMergeOrders = (targetId: string, sourceIds: string[]) => {
      ...
      sources.forEach(src => {
          src.items.forEach(srcItem => {
              ...
              if (existingIdx >= 0) combinedItems[existingIdx] = { ...combinedItems[existingIdx], quantity: combinedItems[existingIdx].quantity + srcItem.quantity };
              else combinedItems.push({ ...srcItem });
          });
      });
      const newTotal = combinedItems.reduce((sum, item) => sum + (item.product.price * item.quantity), 0);
      const updatedOrders = orders.map(o => {
          if (o.id === targetId) return { ...o, items: combinedItems, grandTotal: newTotal, customerName: `${o.customerName} (Merged Bill)` };
          if (sourceIds.includes(o.id)) return { ...o, status: 'merged' as any };
          return o;
      });
  ...
  const handleClaimSection = (section: string) => {
      ...
      const newAlloc: SectionAllocation = { id: `ALC-${Date.now()}`, sectionName: section, waiterId: currentUser.id, waiterName: currentUser.name, timestamp: new Date() };
  ...
  const handleUpdateAllocations = (newAllocations: SectionAllocation[]) => { setAllocations(newAllocations); broadcast('TABLE_UPDATE', newAllocations); };
  ...
  const handleSaveExpense = (expenseData: { itemName: string; department: string; category: string; amount: number; description: string }) => {
    if (editingExpense) setExpenses(prev => prev.map(e => e.id === editingExpense.id ? { ...e, ...expenseData } : e));
    else setExpenses(prev => [{ id: `EXP-${Date.now()}`, timestamp: new Date(), ...expenseData }, ...prev]);
    setIsExpenseModalOpen(false);
    setEditingExpense(null);
  };

  const initiateAddExpense = () => { setEditingExpense(null); setIsExpenseModalOpen(true); };
  const initiateEditExpense = (expense: Expense) => { setEditingExpense(expense); setIsExpenseModalOpen(true); };
  ...
  const handleUpdateUserPhoto = (file: File) => {
    ...
    reader.onloadend = () => {
        ...
        addAuditEntry('STAFF_EDIT', `Updated photo for ${currentUser.name}`);
  ...
  const handleViewChange = (view: AppView) => { setCurrentView(view); setIsSidebarOpen(false); };
  ...
  const handleExportData = () => {
      ...
      link.href = url;
      link.download = `eagle_eyed_backup_${tId}_${new Date().toISOString().split('T')[0]}.json`;
      link.click();
      addAuditEntry('DATA_EXPORT', 'Generated full system archive.');
      notify("Master backup generated successfully.", "success");
  };

  if (isIdleLocked && isAuthenticated) {
    return (<ErrorBoundary><IdleLockScreen onUnlock={() => { setIsIdleLocked(false); setIsAuthenticated(false); setLoginMode('staff'); }} systemConfig={systemConfig}/></ErrorBoundary>);
  }

  if (!isAuthenticated) {
    return (<ErrorBoundary><LoginView onLogin={handleLogin} onClientLogin={handleClientLogin} systemName="EAGLE 🦅 EYED POS" viewMode={loginMode} businessName={systemConfig.name} logoUrl={systemConfig.logo} onSwitchAccount={loginMode === 'staff' ? handleFullLogout : undefined}/><VirtualKeyboard isOpen={isKeyboardOpen} onClose={() => setIsKeyboardOpen(false)} /><ToastContainer notifications={notifications} onDismiss={dismissNotification} /></ErrorBoundary>);
  }

  return (
    <ErrorBoundary>
      <div className="flex h-screen overflow-hidden bg-gray-100 font-sans text-slate-900">
        <Sidebar currentView={currentView} onViewChange={handleViewChange} isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} systemConfig={systemConfig} userRole={userRole} onLogout={handleLogout} onFullLogout={handleFullLogout} currentUser={currentUser} unreadMessageCount={unreadMessageCount} permissions={rolePermissions} onExitImpersonation={currentUser?.id?.startsWith('MASTER_OVERRIDE') ? () => { setSystemConfig(DEFAULT_SYSTEM_CONFIG); setCurrentUser({ id: 'ROOT', name: 'Master Admin', role: 'OWNER', pin: '000000', contact: 'System Master', status: 'active', isProtected: true }); setCurrentView(AppView.TENANTS); } : undefined} />
        <div className="flex-1 flex flex-col min-w-0 relative">
          <Header title={currentView.replace('_', ' ')} onMenuClick={() => setIsSidebarOpen(!isSidebarOpen)} isSidebarOpen={isSidebarOpen} primaryColor={systemConfig.colors.primary} userRole={userRole} businessName={systemConfig.name} onNavigate={setCurrentView} onCalculator={() => setIsCalculatorOpen(true)} onToggleKeyboard={() => setIsKeyboardOpen(!isKeyboardOpen)} onLogout={handleLogout} currentUser={currentUser} />
          <main className={`flex-1 overflow-hidden relative transition-all duration-300`}>
            <div className="h-full overflow-y-auto custom-scrollbar">
                ...
                {currentView === AppView.MASTER_PORTAL && <MasterPortalView onKillSwitch={() => { alert("System Locked"); }} systemConfig={systemConfig} onUpdateConfig={setSystemConfig} businessPages={businessPages} setBusinessPages={setBusinessPages} onPreviewPage={() => {}} onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} isSidebarOpen={isSidebarOpen} onExportData={handleExportData} auditLogs={auditLogs} />}
                {(currentView === AppView.SETTINGS || currentView === AppView.TENANT_SETTINGS) && <SettingsView config={systemConfig} setConfig={(c) => { setSystemConfig(c); addAuditEntry('LOGO_CHANGE', 'Business Branding updated.'); }} userRole={userRole} isMasterView={currentUser?.id === 'ROOT'} currentUser={currentUser} />}
                ...
                {currentView === AppView.HRM && <HRMView staff={staff} setStaff={(s) => { setStaff(s); addAuditEntry('STAFF_EDIT', 'Staff directory modified.'); }} systemConfig={systemConfig} currentUser={currentUser} attendanceRecords={attendance} onUpdateAttendance={(record) => { setAttendance(prev => { const exists = prev.find(r => r.id === record.id); if (exists) return prev.map(r => r.id === record.id ? record : r); return [record, ...prev]; }); }} />}
