...
import KitchenInventoryView from './components/KitchenInventoryView'; // New Import
...
import { Croissant } from 'lucide-react'; // Import icon for Bakery
...
const DEFAULT_SYSTEM_CONFIG: SystemConfig = {
  ...
  receipt: {
    logoUrl: '',
    showLogo: true,
    headerText: 'Welcome to Akamper POS',
...
const App: React.FC = () => {
  ...
  const [systemConfig, setSystemConfig] = usePersistentState<SystemConfig>('systemConfig', DEFAULT_SYSTEM_CONFIG);
  ...
  const handleLogin = (pin: string) => {
      if (pin === systemConfig.master_pin) { 
          ...
          setUserRole('OWNER');
          setCurrentUser({ 
              id: `OWNER-${systemConfig.tenantId}`, 
              name: 'Administrator', 
              role: 'OWNER', 
              pin: systemConfig.master_pin, 
              contact: systemConfig.owner_contact, 
              status: 'active' 
          });
      ...
      if (member) {
          ...
          switch (member.role) {
              ...
              case 'BARISTA': 
                  ...
                  break;
              // CASHIER, MANAGER, OWNER -> DASHBOARD
              case 'CASHIER':
              case 'MANAGER':
              case 'OWNER':
                  setCurrentView(AppView.DASHBOARD);
  ...
  const handleClientLogin = (u: string, p: string) => {
      ...
      if (tenant) {
          ...
          if (new Date() > new Date(tenant.subscriptionExpiry)) {
              ...
              // Proceed anyway but restriction logic is at view level
          }
          ...
          setLoginMode('staff'); 
          notify("Business Found. Enter Staff PIN.", 'info');
      } else {
  ...
  const handleUpdateOrder = (updatedOrder: Order) => {
      ...
      // Auto-trigger kitchen bell if there are new items marked in the update
  ...
  const handleDeleteOrder = (orderId: string, reason?: string) => {
      ...
      if (order) {
          ...
          const record: ReturnRecord = {
              ...
              originalCreator: order.staffName, // CAPTURE ORIGINAL CREATOR HERE
  ...
  const handleItemReturn = (orderId: string, originalTime: Date, items: OrderItem[], totalRefund: number, reason: string) => {
      // Find original order to get the creator
      ...
      const record: ReturnRecord = {
          ...
          timestamp: new Date(),
          authorizedBy: currentUser?.name || 'System',
          originalCreator: originalOrder?.staffName, // CAPTURE ORIGINAL CREATOR HERE
    ...
    return (
      <ErrorBoundary>
        <LoginView 
          ...
          systemName="Akampa POS"
          viewMode={loginMode}
          businessName={systemConfig.name}
          logoUrl={systemConfig.logo}
          onSwitchAccount={loginMode === 'staff' ? handleFullLogout : undefined}
          supportContact={systemConfig.owner_contact}
          supportEmail={systemConfig.support_email}
        />
          ...
          <main className="flex-1 overflow-hidden relative">
            ...
            {currentView === AppView.MASTER_PORTAL && (
                <MasterPortalView 
                    ...
                    systemConfig={systemConfig}
                    onUpdateConfig={(cfg) => { setSystemConfig(cfg); notify("System Config Updated", 'success'); }}
                    businessPages={businessPages}
                    setBusinessPages={setBusinessPages}
                    onPreviewPage={() => {}}
            ...
            {currentView === AppView.SYSTEM_BRANDING && (
                <SystemBrandingView 
                    systemConfig={systemConfig}
                    onUpdateConfig={(cfg) => { setSystemConfig(cfg); notify("System Identity Updated", 'success'); }}
                />
            )}
            {(currentView === AppView.SETTINGS || currentView === AppView.TENANT_SETTINGS) && (
                <SettingsView 
                    config={systemConfig}
                    setConfig={(cfg) => { setSystemConfig(cfg); notify("Settings Saved", 'success'); }}
                    userRole={userRole}
